...
u0.interpolate(Constant(0.0) * exp(- 1000.0 * x[0] * x[0]))
...
def solve_function(u,p, v, q, dt):
    ...
    # ** U part **
    F =  inner(Constant(0.01)*grad(u), grad(p)) * dx + inner( (u-u0)/dt, p) *dx 
    # ** V part **
    
    F += inner(Constant(0.01)*grad(v), grad(q)) * dx + inner( (v-v0)/dt, q)  *dx
