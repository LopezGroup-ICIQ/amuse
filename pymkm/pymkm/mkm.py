...
class MKM:
    ...
    def set_CSTR_params(self,
        ...
        """
        self.reactor.radius = radius
        self.reactor.length = length
        self.reactor.volume = np.pi*radius**2*length
        self.reactor.Q = Q
        self.reactor.m_cat = m_cat
        self.reactor.tau = self.reactor.volume / self.reactor.Q
        self.reactor.s_bet= S_BET
        self.reactor.a_site = A_site
        if verbose == 0:
            print("Reactor volume: {:0.2e} [m3]".format(np.pi*radius**2*length))
            print(
                "Inlet volumetric flowrate : {:0.2e} [m3/s]".format(Q))
            print("Residence time: {:0.2e} [s]".format(Q/length))
            print("Catalyst mass: {:0.2e} [kg]".format(m_cat))
            print(
                "Catalyst surface: {:0.2e} [m2/kg_cat]".format(S_BET))
            print("Active site surface: {:0.2e} [m2]".format(A_site))
            return None
        else:
            return None
    ...
    def apparent_activation_energy(self,
        ...
        eapp, r_squared = calc_eapp(np.asarray(temperature_vector)
                                    , r_ea)
        with open("R_vs_T.txt", "w") as outf:
            outf.write("T [K]"+"\t"+"r [1/s]"+"\n")
            for T,r in zip(temperature_vector, r_ea):
                outf.write(str(T)+"\t")
                for rr in r:
                    outf.write(str(rr)+"\t")
                outf.write("\n")
    ...
    def apparent_reaction_order(self,
                                ...
                                initial_conditions: np.array=None) -> dict:
        """
